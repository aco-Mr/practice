...
public class ConsumerDemoMq {
    ...
    private static final String QUEUE_TOPIC_NAME = "aco_queue_topic1";
...
//            consumerExchangeDirectMessage();
     ...
     * @throws Exception
     */
    public static void consumerWorkQueueMessage() throws Exception{
        // 获取连接
...
//        connection.close();
    ...
    public static void consumerExchangeTopicMessage() throws Exception {
        ...
        DefaultConsumer consumer = new DefaultConsumer(channel){
            ...
            public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException {
                // 消费消息
                log.info("消费者1 ---> 指定{}主题消费消息，交换机名称：{}，消费内容：{}",envelope.getRoutingKey(),envelope.getDeliveryTag(),new String(body));
                // 回答生产者
                channel.basicAck(envelope.getDeliveryTag(),false);
            }
        };
